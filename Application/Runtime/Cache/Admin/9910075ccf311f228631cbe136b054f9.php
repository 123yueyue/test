...
<body>
<div class="title"><h2>管理员信息管理</h2></div>
	...
	<table>
    	<thead>
        	<tr>
            	<th class="num">职工号</th>
                <th class="name">姓名</th>
                <th class="process">所属学院</th>
                <th class="operate">操作</th>
            </tr>
        </thead>
        <tbody>
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
            	<td class="num"><?php echo ($vol["tid"]); ?></td>
                <td class="name"><?php echo ($vol["tname"]); ?></td>
                <td class="num"><?php echo ($vol["id"]); ?></td>
                <td class="operate">
                <input type="checkbox" value="<?php echo ($vol["ttid"]); ?>"/>
                <a href="/tp/index.php?s=/Admin/Teacher/teacher_edit/id/<?php echo ($vol["ttid"]); ?>">编辑</a>
                </td>
...
$(function(){
	...
	$('.add').on('click',function(){
		window.location.href = '/tp/index.php?s=/Admin/Teacher/teacher_add';
	});
});
$(function(){
	//给编辑按钮绑定点击事件
	$('.edit').on('click',function(){
		window.location.href = '/tp/index.php?s=/Admin/Teacher/teacher_update';
	});
});
