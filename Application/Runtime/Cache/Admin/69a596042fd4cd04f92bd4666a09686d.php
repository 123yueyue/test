...
<body>
<div class="title"><h2>选择班级</h2></div>
<form action="" method="post">
	<div class="main">
	    ...
	    <p class="short-input ue-clear">
	    	<label>选择班级：</label>
	        <select name="cname">
	            <option  value="<?php echo ($ccname); ?>"}>--------</option>
	            <?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vol["cname"]); ?>"><?php echo ($vol["cname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	        </select>
	    </p>
	</div>
