	...
	<table>
    	<thead>
        	<tr>
            	<th class="num">学号</th>
                <th class="name">姓名</th>
                <th class="name">所属班级</th>
                <th class="operate">操作</th>
            </tr>
        </thead>
        <tbody>
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
            	<td class="num"><?php echo ($vol["stid"]); ?></td>
                <td class="name"><?php echo ($vol["stname"]); ?></td>
                <td class="num"><?php echo ($vol["cid"]); ?></td>
                <td class="operate">
                <input type="checkbox" value="<?php echo ($vol["id"]); ?>"/>
                <a href="/tp/index.php?s=/Admin/Teacher/student_edit/id/<?php echo ($vol["id"]); ?>">编辑</a>
                </td>
...
$(function(){
	...
	$('.del').on('click',function(){
		...
		//带着参数跳转到del方法
		window.location.href = '/tp/index.php?s=/Admin/Teacher/student_del/id/' + id;
	});
});
...
$(function(){
	...
	$('.add').on('click',function(){
		window.location.href = '/tp/index.php?s=/Admin/Teacher/student_add';
	});
});
$(function(){
	//给编辑按钮绑定点击事件
	$('.edit').on('click',function(){
		window.location.href = '/tp/index.php?s=/Admin/Teacher/student_edit';
	});
});
