	...
	<table>
        ...
        <tbody>
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
            	<td class="num"><?php echo ($vol["pid"]); ?></td>
                <td class="name"><?php echo (str_repeat('&emsp;',$vol["pname"]*2)); echo ($vol["pname"]); ?></td>
                
                <td class="operate">
                <input type="checkbox" value="<?php echo ($vol["pid"]); ?>"/>
                <a href="/tp/index.php?s=/Admin/Teacher/profession_edit/id/<?php echo ($vol["pid"]); ?>">编辑</a>
                </td>
...
$(function(){
	...
	$('.del').on('click',function(){
		...
		//带着参数跳转到del方法
		window.location.href = '/tp/index.php?s=/Admin/Teacher/profession_del/id/' + id;
	});
});
$(function(){
	//给添加按钮绑定点击事件
	$('.add').on('click',function(){
		window.location.href = '/tp/index.php?s=/Admin/Teacher/profession_add';
	});
});
$(function(){
	//给编辑按钮绑定点击事件
	$('.edit').on('click',function(){
		window.location.href = '/tp/index.php?s=/Admin/Teacher/profession_edit';
	});
});
